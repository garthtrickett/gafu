  ...
  (event: ExtendableEvent) => {
    event.waitUntil(
      caches.delete("pages").then(() => {
        console.warn(
          "[SW] Purged legacy pages cache to prevent update lockouts.",
        );
      }),
    );
  },
...
const navigationHandler = async (options: {
  ...
  try {
    ...
    console.error(
      "[SW] Navigation request dispatch failed:",
      error,
    );
...
const audioContentTypePlugin = {
  cacheWillUpdate: async ({
    ...
    console.warn(
      `[SW] Refusing to cache invalid audio response status=${response.status} type=${response.type} contentType=${contentType || "missing"}.`,
    );
    return null;
  },
};
