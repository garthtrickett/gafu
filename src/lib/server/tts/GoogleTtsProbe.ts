...
export const GOOGLE_TTS_PROBE_CONFIG = {
  languageCode: "ja-JP",
...
const errorCode = (cause: unknown): string | number | undefined => {
...
const sanitizeGoogleFailure = (cause: unknown): GoogleTtsProbeError => {
  const message = errorMessage(cause);
  const code = errorCode(cause);
  const credentialFileConfigured =
    Boolean(process.env.GOOGLE_APPLICATION_CREDENTIALS?.trim());

  if (
    code === 16 ||
    code === "16" ||
    /UNAUTHENTICATED|default credentials|invalid_grant/i.test(message) ||
    (credentialFileConfigured &&
      /ENOENT|does not exist|no such file|not found/i.test(message))
  ) {
  ...
  Effect.gen(function* () {
    ...
    if (normalized.length === 0) {
      return yield* Effect.fail(
        new GoogleTtsProbeError({
          kind: "configuration",
          message: "The Google TTS smoke text must not be empty.",
        }),
      );
    }
    ...
    if (Buffer.byteLength(normalized, "utf8") > MAX_INPUT_BYTES) {
      return yield* Effect.fail(
        new GoogleTtsProbeError({
          kind: "configuration",
          message: `The Google TTS smoke text must be ${MAX_INPUT_BYTES} UTF-8 bytes or fewer.`,
        }),
      );
    }
...
export const makeGoogleTtsProbeClient = (): GoogleTtsProbeClient => {
  const client = new textToSpeech.TextToSpeechClient();

  return {
    listVoiceNames: (languageCode) =>
      client.listVoices({ languageCode }).then(([response]) =>
...
export const synthesizeGoogleTtsProbe = (
  ...
  Effect.gen(function* () {
    ...
    const probeClient = client ?? makeGoogleTtsProbeClient();
    ...
    const voiceNames = yield* Effect.tryPromise({
      try: () =>
        probeClient.listVoiceNames(GOOGLE_TTS_PROBE_CONFIG.languageCode),
      catch: sanitizeGoogleFailure,
    });

    if (!voiceNames.includes(GOOGLE_TTS_PROBE_CONFIG.voiceName)) {
      return yield* Effect.fail(
        new GoogleTtsProbeError({
          kind: "configuration",
    ...
    yield* Effect.logInfo(
      `[GoogleTtsProbe] Synthesizing ${GOOGLE_TTS_PROBE_CONFIG.audioEncoding} at speaking rate ${GOOGLE_TTS_PROBE_CONFIG.speakingRate}.`,
    );
    ...
    const content = yield* Effect.tryPromise({
      try: () =>
        probeClient.synthesizeSpeech({
          ...
          voice: {
            languageCode: GOOGLE_TTS_PROBE_CONFIG.languageCode,
            name: GOOGLE_TTS_PROBE_CONFIG.voiceName,
          },
          audioConfig: {
            audioEncoding: GOOGLE_TTS_PROBE_CONFIG.audioEncoding,
            speakingRate: GOOGLE_TTS_PROBE_CONFIG.speakingRate,
          },
        }),
      catch: sanitizeGoogleFailure,
    });
    ...
    yield* Effect.logInfo(
      `[GoogleTtsProbe] Synthesis completed with ${audio.byteLength} bytes.`,
    );
    return audio;
  });