...
export const adaptiveLearningContentAgent = new Agent({
  ...
  instructions: `
    ...
    answer unambiguous. Copy the exact inflected target substring into the target-surface field.
    Target spans use JavaScript UTF-16 offsets and must exactly slice that same target surface; the
    server will recompute the offsets from the copied surface. For checkout/review, explicitly
    classify the situation, surrounding vocabulary,
