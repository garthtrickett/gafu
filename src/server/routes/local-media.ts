...
import { isLoopbackHostname } from "../../lib/shared/loopback.ts";
import { effectPlugin } from "../middleware/effect-plugin.ts";

export { isLoopbackHostname } from "../../lib/shared/loopback.ts";
...
export const makeLocalMediaRoutes = (
  ...
  .use(effectPlugin)
  .post("/audio-envelope", async ({ request, set, runEffect }) => {
    ...
    if (!isLocalMediaHelperEnabled() || !isLoopbackHostname(hostname) || !loopbackOrigin || !hasLocalHeader) {
      set.status = 403;
